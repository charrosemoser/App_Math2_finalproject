%% Ito Numerical Method Comparison for Geometric Brownian Motion (GBM)
...
for jj = 1:length(N_vec)
    ...
    for j = 1:M
        ...
        u_Eul(j, 1) = U0;
        for i = 1:N
            u_Eul(j, i+1) = u_Eul(j, i) + mu*u_Eul(j, i)*dt + sigma*u_Eul(j, i)*dW(i);
        end
        ...
        for i = 1:N
            u_Mil(j, i+1) = u_Mil(j, i) + mu*u_Mil(j, i)*dt + ...
               sigma*u_Mil(j, i)*dW(i) + 0.5*sigma^2*u_Mil(j, i)*(dW(i)^2-dt);
        end
        
        % Ito Runge Kutta
        u_RK(j,1) = U0;
        for i = 1:N
            w = u_RK(j,i) + mu*u_RK(j,i)*dt + sigma* u_RK(j,i)*dW(i);
            wp = u_RK(j,i) + mu*u_RK(j,i)*dt + sigma* u_RK(j,i)*sqrt(dt);
            wm = u_RK(j,i) + mu*u_RK(j,i)*dt - sigma* u_RK(j,i)*sqrt(dt);
            u_RK(j,i+1) = u_RK(j,i) + 0.5*(mu*u_RK(j,i) + mu*w)*dt +...
...
subplot(1,2,1)
hold on
plot(log(N_vec), log(strong_error_Eul), 'b--', 'LineWidth', 2, 'DisplayName', 'Euler-Maruyama')
...
subplot(1,2,2)
hold on
plot(log(N_vec), log(weak_error_Eul), 'b--', 'LineWidth', 2, 'DisplayName', 'Euler-Maruyama')
